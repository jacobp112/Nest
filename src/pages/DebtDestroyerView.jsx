  ...
  return (
    <div className="space-y-3 select-none group">
      <div className="flex justify-between items-end">
        ...
        <span className="text-white font-mono font-bold text-xs sm:text-sm bg-slate-800 px-1.5 py-0.5 sm:px-2 rounded border border-white/5">{unit}{value}</span>
      </div>
...
const DebtRow = ({ debt, strategy, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative flex items-center justify-between p-3 sm:p-4 rounded-2xl border transition-all duration-300 group ${isTarget
    ...
    >
      ...
      <div className="flex items-center gap-4">
        <div className={`h-12 w-12 rounded-2xl flex items-center justify-center border transition-colors ${isTarget ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/20' : 'bg-slate-800 text-slate-400 border-white/5'
          }`}>
          <Icon size={20} />
        </div>
        <div>
          <div className="flex items-center gap-2">
            <h4 className={`font-bold text-xs sm:text-sm ${isTarget ? 'text-white' : 'text-slate-300'}`}>{debt.name}</h4>
            {isTarget && (
              <span className="flex items-center gap-1 text-[8px] sm:text-[9px] font-bold bg-cyan-500/20 text-cyan-400 border border-cyan-500/20 px-1.5 rounded">
                <Target size={10} /> TARGET
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[9px] sm:text-[10px] bg-white/5 px-1.5 py-0.5 rounded text-slate-400 border border-white/5">{debt.rate}% APR</span>
            <span className="text-[9px] sm:text-[10px] text-slate-500">Min: £{debt.min}</span>
          </div>
        </div>
      </div>
      <div className="text-right">
        <p className="font-mono font-bold text-white text-base sm:text-lg">£{debt.balance.toLocaleString()}</p>
        <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-wider">Balance</p>
      </div>
    </motion.div>
  );
};
...
const StrategyCard = ({ title, desc, savings, selected, onClick, icon: Icon }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    className={`w-full text-left p-3 sm:p-4 rounded-2xl border transition-all duration-300 relative overflow-hidden ${selected
  ...
  >
    ...
    <div className="flex justify-between items-start mb-2 relative z-10">
      <div className="flex items-center gap-2">
        ...
        <h4 className={`font-bold text-xs sm:text-sm ${selected ? 'text-white' : 'text-slate-200'}`}>{title}</h4>
      </div>
      {selected && <div className="bg-white text-indigo-600 p-1 rounded-full"><Check size={12} strokeWidth={3} /></div>}
    </div>

    <p className={`text-[10px] sm:text-xs ${selected ? 'text-indigo-100' : 'text-slate-400'} mb-3 relative z-10 leading-relaxed`}>{desc}</p>

    <div className={`inline-flex items-center px-2 py-1 rounded text-[9px] sm:text-[10px] font-bold uppercase tracking-wider relative z-10 ${selected ? 'bg-black/20 text-white' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
      }`}>
...
const SacrificeToggle = ({ icon: Icon, label, amount, active, onToggle }) => (
  <button
    onClick={onToggle}
    className={`flex items-center justify-between w-full p-2 sm:p-3 rounded-xl border transition-all duration-200 ${active
  ...
  >
    <div className="flex items-center gap-3">
      ...
      <div className="text-left">
        <p className={`text-[10px] sm:text-xs font-bold ${active ? 'text-white' : 'text-slate-300'}`}>{label}</p>
        <p className={`text-[9px] sm:text-[10px] ${active ? 'text-emerald-400' : 'text-slate-500'}`}>+ £{amount}/mo</p>
      </div>
    </div>
    <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${active ? 'bg-emerald-500 border-emerald-500 scale-110' : 'border-slate-600 bg-slate-800'
      }`}>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pb-20">
      ...
      <div className="col-span-12 md:col-span-8 space-y-6">
        ...
        <DashboardCard className="relative overflow-hidden min-h-[280px] sm:min-h-[320px] flex flex-col justify-center p-4 sm:p-6 md:p-8 border-0 ring-1 ring-white/5 bg-slate-900/50 group">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-6 backdrop-blur-md shadow-lg shadow-cyan-500/10">
              <Zap size={12} /> Freedom Countdown
            </div>

            <div className="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-4 mb-4">
              <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-display font-bold text-white tracking-tighter drop-shadow-xl">
                {monthsRemaining} <span className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-slate-500 font-sans tracking-normal font-medium">Months</span>
              </h2>
            ...
            <div className="mt-8 max-w-md bg-slate-950/40 p-3 sm:p-4 rounded-2xl border border-white/5 backdrop-blur-md">
              <SmartSlider
        ...
        <DashboardCard title="Attack Strategy" className="h-full border-0 ring-1 ring-white/5 bg-slate-900/50">
          ...
          <div className="mt-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-slate-500">Found Money</span>
              <div className="h-px flex-1 bg-white/10" />
            </div>
            ...
            <button
              ...
              className="w-full mt-6 py-2 sm:py-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-[10px] sm:text-xs uppercase tracking-wider transition-all shadow-lg shadow-cyan-900/20 flex items-center justify-center gap-2 group"
            >
