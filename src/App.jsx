...
export const nestFinanceFirestoreRules = `rules_version = '2';
...
const subtleChipStyles =
  'rounded-pill border border-border/40 bg-surface-muted/70 px-4 py-2 text-sm text-text-secondary shadow-sm';

// (Animations removed; constant no longer needed)
...
const AuthCard = ({ mode, setMode, onLogin, onRegister, loading, error }) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (mode === 'login') {
      ...
      onRegister(form.email, form.password, form.displayName);
    }
  };
...
// NOTE: Legacy onboarding view kept here previously is not used anymore.
// Removing to satisfy lint and reduce bundle size.
/* const OnboardingView = ({ onSubmit, loading }) => {
  const [form, setForm] = useState({ income: '', expenses: '' });

  const handleSubmit = (event) => {
    event.preventDefault();
    onSubmit(Number(form.income) || 0, Number(form.expenses) || 0);
  };

  return (
    <div className={`${cardStyles} w-full max-w-2xl`}>
      <div className="mb-6 flex items-center gap-4">
        <div className="rounded-full bg-emerald-100 p-3 text-emerald-700">
          <Target className="h-6 w-6" />
        </div>
        <div>
          <p className="text-sm font-medium uppercase tracking-wide text-emerald-600">Welcome to Nest</p>
          <h2 className="text-2xl font-semibold text-slate-900">Letâ€™s set the foundation for your financial plan</h2>
        </div>
      </div>
      <p className="mb-6 text-sm text-slate-600">
        Enter your monthly income and recurring expenses. This helps us calculate your baseline savings potential and tailor your dashboard insights.
      </p>
      <form className="grid gap-6 md:grid-cols-2" onSubmit={handleSubmit}>
        <div>
          <label className="mb-2 block text-sm font-medium text-slate-700">Total Monthly Income</label>
          <input
            type="number"
            required
            min={0}
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-slate-700">Total Monthly Recurring Expenses</label>
          <input
            type="number"
            required
            min={0}
...
const serialiseDate = (value, boundary = 'start') =>
...
const calculateNetMonthlySavings = (income, expenses, transactions) => {
...
const DashboardView = ({
  ...
  // Removed unused spendingByCategory map (replaced by spendingTrendData and expenseCategoryBreakdown)
  ...
  const quickAddSectionClasses = `${cardStyles} ${
  ...
  // Optional: compute qualitative insights. Not used in UI currently.
  /* const qualitativeInsights = useMemo(() => {
    const insightsList = [];
    if (totals.net < 0) {
      insightsList.push(
        `You're overspending by ${formatCurrency(Math.abs(totals.net))} each month. Consider dialing back the largest categories first.`,
      );
    } else if (monthlySavings > 0) {
      insightsList.push(
        `At your current pace, you could set aside ${formatCurrency(annualSavingsProjection)} over the next 12 months.`,
      );
    }

    const topCategory = expenseCategoryBreakdown[0];
    if (topCategory && topCategory.percentage > 40) {
      insightsList.push(
        `${topCategory.label} represents ${topCategory.percentage.toFixed(0)}% of your spending. A small adjustment there could free up extra savings.`,
      );
    }

    if (prioritizedGoal) {
      insightsList.push(
        `You're ${prioritizedGoal.months} month${prioritizedGoal.months === 1 ? '' : 's'} away from reaching ${
          prioritizedGoal.name
        } if you keep this momentum.`,
      );
    } else if (goals.length === 0) {
      insightsList.push('Create your first savings goal to give every dollar a mission.');
    }

    if (insightsList.length === 0) {
      insightsList.push('Keep tracking your transactions to uncover actionable insights.');
    }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-pill border border-border bg-surface px-4 py-2 text-sm font-medium text-text-secondary transition hover:border-primary hover:text-text-primary"
            >
              Accounts
            </button>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-pill border border-primary/40 bg-primary/10 px-4 py-2 text-sm font-semibold text-primary transition hover:border-primary hover:bg-primary/15"
            >
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <section className={`${cardStyles} lg:col-span-3`}>
          ...
          <div className="relative z-10 flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className={`${subtleChipStyles} text-left text-sm text-emerald-50/90`}>
        ...
        {hasLinkedAccounts && (
          <motion.section
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.45, ease: 'easeOut' }}
            className="lg:col-span-3 rounded-3xl border border-emerald-500/30 bg-slate-900/80 p-6 shadow-xl shadow-emerald-900/30"
          >
        ...
        {shouldShowFirstTransactionPrompt && (
          ...
          {insightCards.map((card) => (
            <button
              ...
              className={`${cardStyles} relative overflow-hidden text-left transition ${card.accent ? 'bg-emerald-50/90 ring-emerald-200' : ''} hover:shadow-xl`}
            >
        ...
        <section className={`${cardStyles} lg:col-span-2`}>
        ...
        <section className={`${cardStyles} lg:col-span-1`}>
          <div className="flex items-start justify-between">
            <div>
        ...
        <section className={`${quickAddSectionClasses} lg:col-span-2`}>
          ...
          <div className="mt-6 space-y-4">
            <form onSubmit={handleExpenseSubmit} className={expenseFormCardClasses}>
                ...
                <button type="submit" className={`${buttonStyles} w-full`}>Record expense</button>
              </form>
              ...
              <form onSubmit={handleIncomeSubmit} className="space-y-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
                ...
                <button type="submit" className={`${buttonStyles} w-full`}>Record income</button>
              </form>
            </div>
        </section>
          ...
          <section className={`${cardStyles} lg:col-span-1`}>
            ...
            <form onSubmit={handleGoalSubmit} className="mt-4 space-y-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
              ...
              <button type="submit" className={`${buttonStyles} w-full`}>Create goal</button>
            </form>
            <div className="mt-6 space-y-4">
              {goals.length === 0 && (
      ...
      {settingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-surface p-6 shadow-2xl ring-1 ring-border/50">
            <div className="mb-4 flex items-center justify-between">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  onClick={() => setSettingsOpen(false)}
                  className="rounded-pill border border-border bg-surface px-4 py-2 text-sm font-medium text-text-secondary transition hover:border-primary hover:text-text-primary"
                >
                  Cancel
                </button>
                <button
...
const NestFinanceApp = () => {
  ...
  const monthlySavingsEstimate = useMemo(
    ...
    [transactions, userDoc?.recurringExpenses, userDoc?.recurringIncome],
  );
  const [authError, setAuthError] = useState('');
  const [loading, setLoading] = useState(true);
  const [authLoading, setAuthLoading] = useState(false);
  // Removed unused onboardingSaving state to satisfy lint
  ...
  const [justLinkedBank, setJustLinkedBank] = useState(false);
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
        ...
        unsubUser = onSnapshot(userRef, (snapshot) => {
          ...
          setUserDoc(data ? { id: snapshot.id, ...data } : null);
          if (!data || data.recurringIncome === undefined || data.recurringExpenses === undefined) {
            setView('onboarding');
            setOnboardingStep('choice');
            setLinkedAccountsData(null);
            setSelectedBankName('');
            setSelectedBankId('');
            setJustLinkedBank(false);
        ...
        unsubBudgets = onSnapshot(budgetsRef, (snapshot) => {
          setBudgets(snapshot.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() })));
        });
  ...
  const handleRegister = async (email, password, displayName) => {
    ...
    try {
      ...
      if (code.includes('email-already-in-use')) setAuthError('Email already in use. Try logging in.');
      else if (code.includes('weak-password')) setAuthError('Password should be at least 6 characters.');
      else setAuthError('Registration failed. Please check your details and try again.');
    } finally {
  ...
  const handleLogin = async (email, password) => {
    ...
    try {
      ...
      const code = error.code || '';
      if (code.includes('user-not-found')) setAuthError('No account found with this email.');
      else if (code.includes('wrong-password')) setAuthError('Invalid password. Please try again.');
      else setAuthError('Unable to login. Please verify your credentials.');
    } finally {
      setAuthLoading(false);
    }
  };
  ...
  const handleBankSelected = (bankId) => {
    ...
    const mockAccounts = [
      { id: `${bankId || 'bank'}-acc1`, name: `${friendlyName} Current Account`, balance: 1234.56 },
      { id: `${bankId || 'bank'}-acc2`, name: `${friendlyName} Savings`, balance: 5678.9 },
    ];
  ...
  const handleUpdateProfile = async ({ recurringIncome, recurringExpenses }) => {
    if (!user) return;
    const userRef = doc(db, 'users', user.uid);
    await updateDoc(userRef, {
      recurringIncome: Number(recurringIncome) || 0,
  ...
  const handleAddGoal = async (payload) => {
    ...
    const currentGoalCount = Array.isArray(goals) ? goals.length : 0;

    if (currentPlan === 'free' && currentGoalCount >= 1) {
      showPricingModal();
      return;
    }
  ...
  const handleContributeToGoal = async (goal, amount) => {
    ...
    if (goal.isDebt) {
      await updateDoc(goalRef, { currentAmount: increment(-amt) });
      await handleAddTransaction({
        type: 'expense',
        description: `Debt payment: ${goal.name || 'Goal'}`,
        amount: amt,
  ...
  const handleUpsertBudget = async ({ category, amount }) => {
    if (!user) return;
    const existing = budgets.find((b) => b.category === category);
    if (existing) {
      const bRef = doc(db, 'users', user.uid, 'budgets', existing.id);
      await updateDoc(bRef, { amount: Number(amount) || 0, updatedAt: serverTimestamp() });
    } else {
      const bCol = collection(db, 'users', user.uid, 'budgets');
      await addDoc(bCol, { category, amount: Number(amount) || 0, createdAt: serverTimestamp() });
    }
  };

  const handleUpsertAccount = async ({ name }) => {
    if (!user) return;
    const existing = accounts.find((a) => a.name.toLowerCase() === String(name || '').toLowerCase());
    if (existing) return; // no duplicates by name (simple guard)
    const aCol = collection(db, 'users', user.uid, 'accounts');
    await addDoc(aCol, { name: String(name || '').trim(), createdAt: serverTimestamp() });
  };
          ...
          <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:py-10 lg:py-12">
            ...
            {currentNavView === PRIMARY_NAV_VIEW_MAP.dashboard.key ? (
              <DashboardView
                ...
                isInitialLink={justLinkedBank}
                isSyncingTransactions={isSyncingTransactions}
                accountsData={linkedAccountsData || []}
